...
class KnockKnockViewController: UIViewController {
    ...
    @IBOutlet weak var knockKnockView: UIView!
    @IBOutlet weak var jokeText: UILabel!
    
    let jokes: [String] = ["Knock knock.", "Who's there?", "To.", "To who?", "To whom."]
    var iter: Int = 0
    
    @IBAction func jokeFlip(_ sender: Any) {
        iter += 1
        
        UIView.transition(with: knockKnockView, duration: 0.4, options: UIViewAnimationOptions.transitionFlipFromRight, animations: nil, completion: {(_ : Bool) -> Void in
